#!/bin/bash
# IoT bonus — k3d + Argo CD + in-cluster GitLab (Helm) + GitOps from GitLab instead of GitHub.
#
# High-level flow (matches subject: GitLab -> Argo CD -> Kubernetes):
#   1) Ensure k3d cluster and kubeconfig (same cluster name as Part 3: mycluster).
#   2) Create namespaces: argocd, dev, gitlab.
#   3) Install Argo CD (same upstream manifest as Part 3).
#   4) Install GitLab with Helm using lightweight values (ingress/cert-manager/prometheus/runner off).
#   5) Wait for GitLab webservice + toolbox; read initial root password from Kubernetes Secret.
#   6) Create an empty GitLab project (root/iot-gitops) via gitlab-rails runner in toolbox.
#   7) Push this repo’s manifests/ folder to GitLab over HTTP (git on your host + kubectl port-forward).
#   8) Register the in-cluster Git HTTP URL with Argo CD (repo credentials: root + initial password).
#   9) Create (or replace) the Argo CD Application with automated sync, path manifests/, branch main.
#  10) Port-forward Argo CD UI + wil-playground service; curl the app.
#
# Environment overrides (optional):
#   K3D_CLUSTER_ARGS   extra args for "k3d cluster create" (first run only), e.g. '--agents 2'
#   GITLAB_HELM_CHART_VERSION  Helm chart version pin (default set below)
#   GITLAB_HTTP_PF_PORT        localhost port for temporary GitLab port-forward (default 9080)
#   ARGOCD_PF_PORT             localhost port for Argo CD UI (default 8080)
#   APP_PF_PORT                localhost port for wil-playground (default 8888)
...
# ─── ARGO CD ──────────────────────────────────────────────────────────────────
# Argo CD 3.3+ ships an ApplicationSet CRD too large for client-side apply (the
# kubectl.kubernetes.io/last-applied-configuration annotation is capped at 262144 bytes).
# Server-side apply avoids that annotation and is the supported install path.
...
# We use the toolbox pod’s Rails environment to create the project without manual UI clicks.
# This keeps the defense scriptable; if it fails, see reference-commands.txt for manual steps.
...
# Argo CD will later clone the same repo using in-cluster DNS; your laptop pushes via port-forward.
